...
* **First Name & Display Formatting**: Instead of displaying the user's raw email, the account button shows `"Hi, [user_first_name]!"`. The first name is resolved by `WinUIAuthService.CurrentUserFirstName` which checks user metadata keys (`given_name`, `first_name`, `full_name`, `name`) with a fallback to the email local part (split on dots, dashes, or underscores).
...
* **Robust Image Resolvers & Local Caching**: Accounts authenticated via Google OAuth store their profile pictures under the JSON property `picture`, while standard Supabase accounts use `avatar_url`. To ensure smooth, flicker-free, and reliable rendering under WinUI, profile avatars are cached locally on disk (under the `AvatarCache` folder in local storage). The system hashes the avatar URL, downloads the image asynchronously via `HttpClient` (with User-Agent headers to bypass Google request blocks), and streams it locally. An in-memory cache check prevents redundant loading/flickering if the URL has not changed.
...
### 7.6 Scoped Resource Styling & Glass Header Controls
* **Implicit Style Overrides for Third-Party Controls**: WinUI third-party controls (such as Syncfusion's `SfScheduler`) often lack properties to style internal sub-elements (like the Today, Previous, and Next navigation buttons). We resolved this by defining an implicit `<Style TargetType="Button">` scoped strictly inside `<scheduler:SfScheduler.Resources>`. This applies custom heights (`32px`), vertical alignment (`Center`), margins, padding, and corner radii (`CornerRadius="6"`) to the internal buttons without affecting buttons elsewhere in the app.
* **Unified Visual Center & Sizing**: The native buttons are styled as standalone glass buttons using the same background (`AppGlassColorBrush`) and border (`AppGlassBorderColorBrush`) as the dashboard widgets. To align them with our custom overlay view switcher (Day/Week/Work/Month segmented capsule), the overlay container's height is locked to `32px` with a matching `CornerRadius="6"` and `Padding="2"`, and its internal buttons are scaled to `26px` in height. This aligns all header controls on a mathematically identical vertical center line.
* **Merged Sidebar Collapse calendar Button**: The accounts sidebar collapse button and the calendar header icon were merged into a single circular `40x40px` glass button (`CornerRadius="20"`, `BorderThickness="1"`) displaying the calendar icon (`&#xE787;`). This removes visual clutter and acts as a cohesive sidebar toggle button.
