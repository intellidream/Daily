...
3. **Interactive `AppIntent`**: Each interactive button runs in the widget extension process, executing mutations via `WidgetDataCoordinator` and notifying `WidgetCenter.shared.reloadAllTimelines()`. Queued changes are persisted locally and synchronized with Supabase when the main app runs.
