...
- **Offline-First UI & Local Caching**: To eliminate cold-start loading spinners and provide instant interactivity, the app implements an offline-first architecture via `@zos/fs`. Habit data (`habits_cache.json`) and previous telemetry baselines (`telemetry_cache.json`) are immediately parsed and rendered to the UI on load. A background `fetchData()` routine silently pulls fresh data and dynamically re-renders the arcs, indicated to the user by a non-blocking `🔄 Syncing...` indicator.
- **Delta Telemetry Deduplication**: To prevent redundant database rows, the wearable tracks the exact value of the last transmitted sensor (e.g., `tCache.heart_rate`). It only appends data to the sync payload if the newly read sensor value explicitly differs from the cached baseline. For array-based sleep stages, the app hashes the stage via `${start_time}-${end_time}-${stage_type}` and maintains a rolling array of up to 100 transmitted keys to enforce strict idempotency during hourly re-syncs.
- **QJSC Compiler Stability Fix (ES2020 Avoidance)**: The Zepp OS 3.0 QJSC compiler has a severe, undocumented limitation where utilizing modern ES2020 Optional Chaining (`?.`) causes the `zeus preview` compilation command to silently hang indefinitely. The architecture strictly enforces defensive JS evaluations (e.g., `if (obj && obj.value)`) across all device-side code to guarantee build stability across macOS and Windows environments.
- **Swiper Stale Texture Bug**: When utilizing `SCROLL_MODE_SWIPER`, Zepp OS exhibits a rendering bug where calling `.setProperty` on UI widgets located on off-screen pages causes them to retain stale graphical textures upon returning to them (e.g., histograms showing outdated data after a background sync). To mitigate this, the architecture implements a global UI redraw loop via a 2-second `setInterval` that forces constant repaints of all charts and arcs.
- **Zeus CLI Device Selection Quirk**: When deploying to a physical device via `npx zeus preview`, selecting the target watch model from the top "Suggested/Recently Used" section frequently results in cryptic `Download failed : error code null` or `Device does not support` errors, despite Developer Mode being enabled. The confirmed workaround is to completely ignore the suggested list, scroll down to the full catalog, and manually select the target device every time.
- **Cross-Platform UI Parity (Emojis & Layout)**: To maintain a consistent visual language across the watchOS and Zepp OS implementations, the UI heavily leverages universal emojis for status indicators (e.g., 🔥 for Cigarettes, ⚡ for Heated Tobacco, 💧 for Water). The UI is strictly optimized for small screens, manually dropping Y-coordinates to avoid status bar occlusion and utilizing large, high-contrast arcs (Progress Rings).
- **Asset Compilation and Platform Overrides**: Zepp OS uses highly strict platform-specific asset resolution rules. When compiling, it looks for the app icon inside `/assets/common.s/` (Square), `/assets/common.r/` (Round), and `/assets/common.b/` (Band), taking precedence over the root `/icon.png`. Furthermore, the `png2tga` compiler will silently drop images that do not contain a 32-bit RGBA (Alpha) channel, leading to missing icons at runtime. All app icons must be exported strictly as 32-bit RGBA PNGs (even if fully opaque) and saved at exact resolutions (e.g., `124x124` for Square, `248x248` for Round).
- **Multi-Device UI Adaptation (Amazfit Balance Round & Active 2 Square)**: Supported via `@zos/device` (`getDeviceInfo()` with `SCREEN_SHAPE_ROUND`). Dynamically chooses between `ROUND_CONFIG` (480x480 circular, scaled arcs, wider touch buttons, centered bottom texts, expanded histograms) and `SQUARE_CONFIG` (390x450 square, preserving 1:1 original layout for Active 2). For complete details, see [ZeppOS-MultiDevice-Balance-Active2.md](file:///Users/mihai/Source/Daily/Docs/Features/ZeppOS-MultiDevice-Balance-Active2.md).
- **Comprehensive Deployment & Delivery Guide**: For step-by-step instructions on setting up from a clean machine, unlocking Developer Mode in the Zepp smartphone app, installing via `npx zeus preview` QR code, and troubleshooting, see [ZeppOS-Amazfit-Deployment-Guide.md](file:///Users/mihai/Source/Daily/Docs/Features/ZeppOS-Amazfit-Deployment-Guide.md).
