...
- **Narrow Width Window Adaptation (Responsive Header UI)**: Added a `SizeChanged` event handler to adapt the custom scheduler header components and sidebar state when the application window is resized to narrow widths (`ActualWidth < 800`):
  - **Connected Accounts Sidebar Auto-Collapse**: Collapses the connected accounts sidebar panel (by setting `ToggleSidebarBtn.IsChecked = false`) when resizing to a narrow screen, freeing up horizontal space for the main scheduler view.
