...
### 5.9 Dashboard Widget Glassy Styling (June 2026)
- **Glassy Theme Adaptations in Light Mode**: Migrated container and list item card backgrounds and borders in `CalendarWidgetControl.xaml` from opaque white brushes (`CardBackgroundFillColorDefaultBrush` / `CardStrokeColorDefaultBrush`) to semi-transparent theme-consistent glassy brushes (`AppGlassSubColorBrush` / `AppGlassBorderColorBrush`).
- **Improved Visual Uniformity**: Applied visual alignment to all widget containers (`SmallContainer`, `NormalContainer`, `TallContainer`, `LargeContainer`) and the left-side desk-calendar date card in the Large widget view. This ensures elegant transparency in both Light and Dark modes.
- **Current & Future Items Filtering**: Updated the widget data load query to skip past events whose end time is before the current UTC time (`ev.End < DateTime.UtcNow`) and skip past uncompleted tasks whose due date is before today (`td.DueDate.HasValue && td.DueDate.Value.Date < DateTime.UtcNow.Date`), guaranteeing only current or future entries are shown.
- **Origin Account Color Mapping**: Programmed the widget to query all calendar accounts dynamically and map the event/todo colors to the specific color of the originating calendar account instead of defaults.
- **Circular Badging with Icon Indicators**: Replaced checkboxes, ellipse color dots, and thin vertical border strips in *all* containers (`SmallContainer`, `NormalContainer`, `TallContainer`, `LargeContainer`) with circular badges colored according to the source account:
  - **Events**: Displays a Segoe Fluent Icons calendar glyph (`&#xE787;`) inside the circle.
  - **Todos**: Displays a Tabler Icons check glyph (`&#xEB28;`) inside the circle, functioning as an interactive button that marks the task completed upon click.
- **Scheduler Header & Time Ruler Visual Distinction**: Overrode `SyncfusionViewHeaderControlBackground`, `SyncfusionViewHeaderBaseBackground`, and `SyncfusionTimeRulerCellBackground` with `{ThemeResource AppGlassSubColorBrush}` in [CalendarDetailPage.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/CalendarDetailPage.xaml). This applies a subtle transparent overlay to the days-of-week header bar and the hours-line column on the left, making them visually distinct from the rest of the scheduler grid while adapting elegantly to both Light and Dark themes.

### 5.10 Month View Left Border & Collapsible Sidebar Expansion Fixes (June 2026)
- **Collapsible Sidebar Expansion Animation Fix**: Configured the sidebar width animation (`widthAnimation` targeting `SidebarContainer`) in [CalendarDetailPage.xaml.cs](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/CalendarDetailPage.xaml.cs) with `EnableDependentAnimation = true`. Because animating a FrameworkElement's `Width` affects layout, it is classified as a dependent animation in WinUI and is ignored unless explicitly enabled. Enabling it allows the sidebar panel to expand and collapse smoothly.
- **Month View Left Border Restoration**: Wrapped the `SfScheduler` control in [CalendarDetailPage.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/CalendarDetailPage.xaml) inside a `Border` container configured with `BorderThickness="1,0,0,0"` and `BorderBrush="{ThemeResource AppGlassSubColorBrush}"`. Since the custom month cells only render right and bottom borders (`BorderThickness="0,0,1,1"`), the leftmost cells lacked a left border. The new wrapper draws a clean vertical boundary line on the left side of the calendar grid matching the style and color of the cell borders.

### 5.11 Empty State Visual Suggestiveness & Sidebar Auto-Expansion (June 2026)
- **Connected Accounts Sidebar Auto-Expansion**: Configured `LoadAccountsAsync()` in [CalendarDetailPage.xaml.cs](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/CalendarDetailPage.xaml.cs) to automatically toggle the sidebar open (`ToggleSidebarBtn.IsChecked = true`) and expand the "Add New Account" Expander (`AddAccountExpander.IsExpanded = true`) if the list of connected accounts is empty. The "Add New Account" Expander is programmatically collapsed once accounts are present.
- **Connected Accounts Empty Placeholder**: Added a centered placeholder layout (`NoAccountsPlaceholder` StackPanel) inside [CalendarDetailPage.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/CalendarDetailPage.xaml) that displays a clean instruction message ("No accounts connected yet. Use the buttons below to add your calendars.") and a calendar icon when no accounts are linked, which is hidden automatically when at least one account is connected.
- **Calendar Widget Empty State Iconography**: Updated the empty states across all four widget layout views (**small**, **wide/normal**, **tall**, and **large**) in [CalendarWidgetControl.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Controls/CalendarWidgetControl.xaml) to display a calendar brand icon (`&#xE787;`) styled in `SystemAccentColor` directly above/beside the status text, matching the accent color of the widget title.

### 5.12 Event Drag-and-Drop Limitations & Editor Research (June 2026)
- **Drag-and-Drop Limitations**:
  - Resizing or moving appointments (drag-and-drop) on the scheduler is currently purely visual. There are no event handlers (such as `AppointmentDrop` or `AppointmentDragOver`) bound in [CalendarDetailPage.xaml.cs](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/CalendarDetailPage.xaml.cs) to intercept position changes.
  - Additionally, [ICalendarService](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Services/ICalendarService.cs) is strictly read-only for event objects: it fetches remote events and stores them locally in the `LocalCalendarEvent` database cache, but contains no endpoints or query interfaces for creating, updating, or deleting calendar event objects. Drag changes are therefore retained only in-memory inside the scheduler UI layer and are discarded on page navigation, view changes, or refreshes.
- **Built-in Appointment Editor Dialog Behavior**:
  - The calendar detail view relies on Syncfusion `SfScheduler`'s default built-in editor dialog, which opens by default on cell double-tap/double-click.
  - When a user saves an appointment in the built-in editor, the control adds it to the bound `Appointments` collection in memory, but because no listener is registered for `Appointments.CollectionChanged` (or `AppointmentEditorClosing`) to commit it to a database or call a cloud API, the created event is lost upon reload.
  - The default editor lacks the concept of the application's multi-account structure (Google, Microsoft, Yahoo) and does not display or allow selection of the calendar account in which the event should be created.

#### 5.13 Cross-Device Account Syncing & Deduplication (June 2026)
  - **Deterministic Account IDs**: Created `Daily_WinUI.Helpers.UuidHelper.GenerateDeterministicGuid()` to generate mathematically consistent UUIDv5s based on `UserId_AccountType_Email` instead of random GUIDs.
  - **Cross-Machine Sync**: This ensures that when calendar connections are synced via Supabase, connecting the same account on a second machine (or fetching the encrypted tokens from the cloud) resolves to the exact same calendar account ID, preventing duplicate calendar entries in the UI and seamlessly sharing access across machines (including Yahoo App Passwords).
  - **UI State Synchronization**: Added `CustomName`, `IdentifiedName`, and `DisplayOrder` properties to the remote `Supabase` calendar models. This ensures that custom names and ordering preferences configured on one device are propagated accurately to the local SQLite database on all other synced devices.
  - **Self-Healing Full Sync**: Implemented auto-heal logic inside `SyncService.cs`. If the local SQLite database evaluates as completely empty for connected accounts, it natively bypasses the delta-sync `lastPull` optimization to force a full-pull from Supabase, organically fetching all deterministic accounts down from the cloud instantly.
  - **Soft-Delete Sync Architecture**: Migrated Calendar Account deletion from hard-deletes to a cross-device Soft-Delete approach. When an account is removed locally, its `IsDeleted` property is flipped to `true` (mapped to `is_deleted` in Supabase) and events are physically swept locally. On the next sync cycle, this state is pushed to the cloud and subsequently pulled by other machines, which gracefully sweep their local SQLite cache, effectively enabling multi-device deletion propagation. In-memory `!IsDeleted` filtering prevents SQLite from discarding valid accounts whose newly-added boolean columns fallback to `NULL`.
