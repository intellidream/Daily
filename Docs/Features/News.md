...
- **Compliant Background Scraper**:
  ...
  - **Forced WebView2 Extraction for Medium Links**: Bypasses the anonymous HttpClient fast-path in `OpenReaderViewAsync` for all Medium links (URLs containing `medium.com`). This forces the reader to use the WebView2 fallback rendering path, leveraging the authenticated cookie store to extract the full member-only article text instead of just a public summary.
- **Author RSS Subscription Action**:
  - Added a custom "+" button inside [RssFeedDetailPage.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/RssFeedDetailPage.xaml#L205-L220) for Medium items (any article URL containing `medium.com`).
...
- **Medium Integration Polish & Enhancements**:
  - **Dashboard News Widget Support**: Injected the "Medium Reading List" and **"All News"** entries into the [RssFeedWidgetControl.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Controls/RssFeedWidgetControl.xaml) feed selection menu, implementing a hidden `BackgroundWebView` scraper for the reading list, a consolidated `LoadAllNewsFeedAsync` compiler for all news, and updating cycling, loading, and refreshing logic.
  - **Widget Loader Spinner**: Tracked custom background loading operations via a local `_isWidgetLoadingCustomFeed` flag to keep the loading ProgressRing active, bypassing the asynchronous race conditions from standard RSS background tasks.
  - **Round-Robin Selector (Widget-only)**: Aggregated the "All News" widget articles by selecting the first article from each subscription feed sequentially. Sorts them chronologically and truncates them to 5, ensuring maximum variety across all categories in the widget.
  - **Author/Publication Splitting & Style**: Refactored both direct RSS parsing (`RssFeedService.cs`) and scraped list parsing to split `"Author in Publication"` string formats into distinct `Author` and `PublicationName` fields. Styled the author metadata line in the details list view with a Tabler user icon (`&#xEB4D;`) and medium font weight.
  - **Image Quality Optimization**: Integrated `OptimizeMediumImageUrl` inside [RssFeedService.cs](file:///c:/Users/mihai/source/repos/Daily/Services/RssFeedService.cs) to parse and replace Miro CDN thumbnail resize/format segments (e.g. `v2/resize:fill:160:112/...`) with `v2/resize:fit:800`, ensuring high-resolution image previews.
  - **Description Clean-up & Subtitle Scraping**: Added an HTML tag stripper and HTML entity decoder (`StripHtmlTags` inside [RssFeedService.cs](file:///c:/Users/mihai/source/repos/Daily/Services/RssFeedService.cs)) to clean direct RSS feed descriptions. Extended the reading list JavaScript scraper to capture subtitles from `<h3>` elements or non-metadata `<p>` tags, so that Reading List items also display clean description summaries in lists.
  - **Feed Selection Bug Fix**: Fixed a bug in [RssFeedDetailPage.xaml.cs](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/RssFeedDetailPage.xaml.cs#L1121-L1145) where switching from the Medium Reading List to another Medium feed (such as an author subscription) failed to reload the articles. Resolved this by tracking the previous selection in `_currentLocalFeed` and forcing a refresh if it differs from the newly selected feed.
  - **Cache Pollution & View Overwrite Bug Fix (Detail View-only)**: Fixed a bug where viewing "All News" or "Medium Reading List" in the Detail View got corrupted or overwritten by background updates of previously selected standard feeds. Added guards to `RssService_OnItemsUpdated` to prevent caching custom view lists in standard feed slots inside `_allFeedsCache`, and updated `UpdateArticles()` to return early and prevent view state corruption when displaying custom feeds. (Note: The Detail page continues to display sequential groups of 2 articles per feed in its large list, preserving the design, while the widget implements round-robin).
  - **WP JSON Excerpt Formatting (e.g. ZonaIT)**: Cleaned WP JSON excerpts (WordPress feed summaries) by stripping HTML tags and decoding HTML entities using `StripHtmlTags` inside [RssFeedService.cs](file:///c:/Users/mihai/source/repos/Daily/Services/RssFeedService.cs), and added a fallback to the clean content snippet if the excerpt is missing or empty. This prevents raw HTML formatting elements (like `<p>` tags) from leaking into list summaries (e.g., ZonaIT feed entries).

### 6.4 RSS Subscriptions Seeding & Matching Improvements (June 2026)
- **Supabase-First Seeding Verification**: Updated the startup RSS seeder logic in [SeederService.cs](file:///c:/Users/Mihai/source/Repos/Daily/Services/SeederService.cs) to check the cloud backplane (Supabase) first. Default feeds are seeded to the cloud only if the user's remote RSS subscriptions list is completely empty.
- **Deterministic URL-Based GUIDs**: Replaced hardcoded default feed GUIDs (e.g. `d0000000-...` constants) with stable, programmatic GUIDs generated deterministically using MD5 hashes of their URLs via `GenerateGuid(url)`. This ensures IDs are consistent across runs and installations without manual code maintenance.
...
### 6.5 RSS Subscriptions Drag-and-Drop Reordering & Sync Fixes (June 2026)
- **UI Drag Handle Modernization**: Enabled drag-reordering (`CanDragItems="True"`, `SelectionMode="Single"`) on `CurrentFeedsListView` in [FeaturesPage.xaml](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/FeaturesPage.xaml), replacing the Move Up/Down arrow buttons with a sleek drag grip handle (`&#xE700;` icon). Selection highlights are hidden by clearing selection inside the `SelectionChanged` handler.
- **Debounced Collection Reorder Tracking**: Implemented a 300ms debounced reorder saving window inside `SaveOrderAsync` in [FeaturesPage.xaml.cs](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/FeaturesPage.xaml.cs) to wait for intermediate collection drag-and-drop actions to complete before writing indices sequentially to SQLite and triggering sync.
- **Supabase Schema Column Alignment**: Aligned C# model to database schema by changing the `DisplayOrder` database column mapping in [RssModels.cs](file:///c:/Users/mihai/source/repos/Daily/Models/RssModels.cs) from `"idx"` (which was missing on Supabase, causing push requests to fail with a `PGRST204` error) to `"display_order"`.
- **Overwrite Protection on Pull**: Refactored `PullRssSubscriptionsAsync` in [SyncService.cs](file:///c:/Users/mihai/source/repos/Daily/Services/SyncService.cs) to fetch local records first: if a local record has unsynced updates (`SyncedAt == null`) and its local update time is newer than or equal to the remote record, the local modifications are protected and not overwritten by the remote pull.
- **Flicker-free Smart In-place Updates**: Replaced the legacy `.Clear()` and rebuild logic inside `LoadFeedsListAsync()` in [FeaturesPage.xaml.cs](file:///c:/Users/mihai/source/repos/Daily/WinUI/Daily.WinUI/Views/FeaturesPage.xaml.cs) with a smart in-place update loop. It removes deleted items, inserts new ones, updates existing item properties, and calls `_feedsList.Move()` only when items shift. This prevents visual list flashing and preserves the fallback icon states of the items.
- **Saved Articles Deduplication (Read Later & Favorites)**: 
  - **Deterministic GUIDs**: Updated [RssArticleService.cs](file:///c:/Users/mihai/source/repos/Daily/Services/RssArticleService.cs) to generate deterministic GUIDs for saved articles based on `UserId + ":" + ArticleUrl + ":" + ArticleType` instead of random GUIDs. Uses `Encoding.UTF8.GetBytes(input)` to ensure 100% consistent GUIDs across all machines, regardless of local system ANSI codepage settings.
  - **Self-Healing Startup**: Added `DeduplicateSavedArticlesAsync()` called on startup initialization to group existing duplicates, merge them into the canonical deterministic GUID row, and mark the redundant random GUID rows as deleted so they sync and soft-delete from Supabase.
  - **Reactive UI Refresh on Sync**: Exposed `OnSavedArticlesPulled` in `ISyncService`. Subscribed to it inside `RssArticleService` so that when a background sync downloads a new saved article from another machine, the UI cache instantly refreshes, preventing "ghost" unsaved visual states that cause users to create duplicates.
