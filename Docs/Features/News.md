...
- **Saved Articles Deduplication (Read Later & Favorites)**: Updated [RssArticleService.cs](file:///c:/Users/mihai/source/repos/Daily/Services/RssArticleService.cs) to generate deterministic GUIDs for saved articles based on `UserId + ":" + ArticleUrl + ":" + ArticleType` instead of random GUIDs. Added `DeduplicateSavedArticlesAsync()` called on startup initialization to group existing duplicates, merge them into the canonical deterministic GUID row, and mark the redundant random GUID rows as deleted so they sync and soft-delete from Supabase.
